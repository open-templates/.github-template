---
okf_version: "0.1"
---

# .github-template

OKF knowledge bundle for the public GitHub organization meta-repository (`.github`).

## Documentation

* [README.md](README.md) - quick start
* [INSTRUCTIONS.md](INSTRUCTIONS.md) - init and org profile governance
* [templates/ABOUT_TEMPLATES.md](templates/ABOUT_TEMPLATES.md) - adopter file manifest

## Features

* [01 — Purpose](specs/features/01-purpose.md) - public org meta-repo goals
* [02 — Init workflow](specs/features/02-init-workflow.md) - personalize from templates/
* [03 — Org profile](specs/features/03-org-profile.md) - public visitor-facing README
* [04 — GitHub automation](specs/features/04-github-automation.md) - Dependabot, CODEOWNERS, issue templates

## Related template

* [`.github-private-template`](https://github.com/open-templates/.github-private-template) — optional member-only org profile

## History

* [specs/log.md](specs/log.md)
